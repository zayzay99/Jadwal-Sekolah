    ...
    <div class="flex flex-col items-center">
      <div class="relative mb-4">
        <div class="cursor-pointer group" onclick="document.getElementById('profile_picture_input').click();" title="Klik untuk ganti foto">
          <img src="{{ $siswa->profile_picture ? asset('storage/' . $siswa->profile_picture) : asset('storage/Default-Profile.png') }}"
               alt="Foto Profil"
        ...
        <form id="profile-pic-form" action="{{ route('siswa.profile.update') }}" method="POST" enctype="multipart/form-data" class="hidden">
          @csrf
      ...
      <p class="text-gray-500 text-sm mb-5">Terima kasih sudah belajar hari ini 👩‍🏫🌼</p>
      ...
      <div class="grid sm:grid-cols-2 gap-5 w-full">
        <div class="bg-[#d9eaf5] p-5 rounded-lg text-[#274c77] shadow-sm">
          ...
          <p><strong>Nama:</strong> {{ $user?->nama ?? '-' }}</p>
          <p><strong>NIP:</strong> {{ $user?->nis ?? '-' }}</p>
